...
![真机实录](docs/fold-demo-real.gif)

上面这段录的是**真实桌面**，不是合成画面：浏览器里打开本仓库页面，合盖时整个页面被实时弯折虚化。注意顶部的文件名已经糊成一片，底部的 `LICENSE`、`README.md`、`build.sh` 仍然清晰可读——这就是「越靠上越虚」的梯度。

高清原片（1280×832，4.8 秒，无音轨）：[docs/fold-demo.mp4](docs/fold-demo.mp4)
